...
Express + TypeScript API for Baton: receives webhooks from business platforms, verifies them, runs them through the rule engine, and launches Docusign Maestro workflows via SQS-backed background workers.
...
| 4 | `workflows` | Synced Docusign Maestro workflow definitions |
...
| Every 30 sec | Instance sync | Sync running instance statuses from Maestro (rate-limit aware) |
