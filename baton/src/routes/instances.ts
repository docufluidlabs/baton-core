...
// ─── POST /api/instances/:id/report — Create ClickUp support ticket ─

router.post('/:id/report', requireMember, async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!env.SUPPORT_TICKET_TOKEN || !env.SUPPORT_TICKET_LIST_ID) {
      res.status(501).json({ error: 'Support ticket reporting is not configured' });
      return;
    }
    ...
    if (instance.status !== 'failed') {
      res.status(400).json({ error: 'Only failed instances can be reported' });
      return;
    }

    // User-provided context from the modal
    const userTitle = (req.body.title as string || '').trim();
    const userDescription = (req.body.description as string || '').trim();
    const userEmail = (req.body.email as string || '').trim();

    // Build ClickUp task
    const inputJson = instance.inputData && Object.keys(instance.inputData).length > 0
    ...
    const durationMs = instance.startedAt && instance.completedAt
    ...
    // User context comes first — most important for triage
    if (userDescription) {
      sections.push(`## Context`, `> ${userDescription}`, ``);
    }
    ...
    sections.push(
      ...
      `- **Failed:** ${fmtDate(instance.completedAt)}`,
      `- **Duration:** ${durationStr}`,
      `- **Retries:** ${instance.retryCount ?? 0}${instance.retryMaxAttempts ? ` / ${instance.retryMaxAttempts}` : ''}`,
      ``,
      `## IDs`,
      `- **Instance:** \`${instance.id}\``,
      `- **Instance Name:** ${instance.instanceName}`,
      `- **Maestro ID:** ${instance.maestroInstanceId ? `\`${instance.maestroInstanceId}\`` : '—'}`,
      `- **Automation Rule:** ${instance.triggerRuleId ? `\`${instance.triggerRuleId}\`` : '—'}`,
    );
    ...
    sections.push(
      ``,
      `---`,
      `_Reported by ${req.auth!.userId}${userEmail ? ` (${userEmail})` : ''} · Auto-generated by Baton_`,
    );

    const description = sections.join('\n');

    const taskName = userTitle
      ? `[Baton] ${userTitle}`
      : `[Baton] Failed: ${instance.instanceName} — ${instance.errorStep || instance.errorMessage?.slice(0, 60) || 'unknown'}`;

    const response = await fetch(`https://api.support.example.com/api/v2/list/${env.SUPPORT_TICKET_LIST_ID}/task`, {
      method: 'POST',
      headers: {
        Authorization: env.SUPPORT_TICKET_TOKEN,
    ...
    if (!response.ok) {
      ...
      logError('ClickUp API error', null, { status: response.status, body: err });
      res.status(502).json({ error: 'Failed to create support ticket' });
      return;
    }

    const task = await response.json() as { id: string; url: string };

    logInfo('Support ticket created', { instanceId: instance.id, taskId: task.id, taskUrl: task.url });
    res.json({ taskId: task.id, taskUrl: task.url, message: 'Support ticket created' });
  } catch (error) {
    next(error);
  }
});
