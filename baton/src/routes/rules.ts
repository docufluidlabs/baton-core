...
// TODO: TEMP — one-time prod fix, comment out after use
router.post('/:id/resolve-failures', requireMember, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const orgId = req.auth!.orgId;
    const ruleId = req.params.id;
    const docClient = getDocClient();

    // Fetch all failed instances for this rule
    const result = await docClient.send(new QueryCommand({
      TableName: TableNames.WORKFLOW_INSTANCES,
      IndexName: 'orgId-startedAt-index',
      KeyConditionExpression: 'orgId = :orgId',
      FilterExpression: 'triggerRuleId = :ruleId AND #st = :status',
      ExpressionAttributeValues: { ':orgId': orgId, ':ruleId': ruleId, ':status': 'failed' },
      ExpressionAttributeNames: { '#st': 'status' },
    }));

    const failedInstances = (result.Items || []) as WorkflowInstance[];
    const now = new Date().toISOString();

    // Mark each failed instance as completed
    await Promise.all(failedInstances.map((inst) =>
      docClient.send(new UpdateCommand({
        TableName: TableNames.WORKFLOW_INSTANCES,
        Key: { id: inst.id },
        UpdateExpression: 'SET #st = :status, completedAt = :now, countedAsFailed = :false',
        ExpressionAttributeValues: { ':status': 'completed', ':now': now, ':false': false },
        ExpressionAttributeNames: { '#st': 'status' },
      })),
    ));

    // Reset failureCount and successRate on the automation rule
    const ruleResult = await docClient.send(new GetCommand({
      TableName: TableNames.AUTOMATION_RULES,
      Key: { id: ruleId },
    }));
    const rule = ruleResult.Item;
    if (rule) {
      const total = rule.timesTriggered || 0;
      await docClient.send(new UpdateCommand({
        TableName: TableNames.AUTOMATION_RULES,
        Key: { id: ruleId },
        UpdateExpression: 'SET failureCount = :zero, successRate = :rate, updatedAt = :now',
        ExpressionAttributeValues: { ':zero': 0, ':rate': total > 0 ? 100 : 0, ':now': now },
      }));
    }

    logInfo('Resolved failed instances for rule', { ruleId, count: failedInstances.length });
    res.json({ resolved: failedInstances.length });
  } catch (error) {
    next(error);
  }
});
