...
registry.registerPath({
  method: 'post',
  path: '/api/instances/{id}/report',
  tags: [TAG],
  summary: 'Create a ClickUp support ticket for a failed instance',
  security: SECURITY,
  request: {
    params: idParam,
    body: {
  ...
  responses: {
    200: {
      ...
      content: {
        'application/json': {
          schema: z.object({ taskId: z.string(), taskUrl: z.string().url(), message: z.string() }),
        },
      },
    },
    400: commonErrorResponses[400],
    401: commonErrorResponses[401],
    404: commonErrorResponses[404],
    502: { description: 'ClickUp API error' },
    501: { description: 'Support ticket reporting not configured' },
  },
});
