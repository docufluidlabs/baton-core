 ...
 *
 * Editing an existing automation never re-requires the secret.
 */
 ...
 *                       Saved yet (new automation). This is the case worth
 *                       surfacing as a hint — nothing else looks wrong.
 */
...
export function automationSaveBlockReason(s: AutomationSaveState): AutomationSaveBlockReason {
  ...
  // New automation: the webhook secret must be saved first.
  if (!s.editingAutomation && !s.secretSaved) return 'secret-unsaved';
  return null;
}
