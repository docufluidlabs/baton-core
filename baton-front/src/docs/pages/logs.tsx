  ...
  return (
    <>
      ...
      <Lead>
        ...
        automation handled, and the Instances panel shows every Maestro run a workflow produced.
      </Lead>
      ...
      <p>
        ...
        <strong>Route</strong> (launch the Maestro workflow). One Action covers both steps from end to end.
      </p>

      <FlowStrip>
        <FlowNode k="Step 1" t="Verify" d="Check the webhook's signature against the automation's secret." />
        <FlowNode k="Step 2" t="Route" d="Match the parameters and launch the target Maestro workflow." />
      </FlowStrip>
      ...
      <ul>
        ...
        <li>The <strong>Webhook ID</strong>, <strong>Action ID</strong>, and <strong>Maestro Instance ID</strong> — all copyable</li>
      ...
      <TableWrap>
        <table>
          ...
          <tbody>
            ...
            <tr><td><Badge color="red">verification_failed</Badge></td><td>The signature didn't match. Nothing was sent to Maestro.</td></tr>
            ...
            <tr><td><Badge color="red">upstream_error</Badge></td><td>Maestro returned an error; retries are in progress or exhausted.</td></tr>
          </tbody>
        </table>
      </TableWrap>
      ...
      <ul>
        ...
        <li>The inputs Maestro received — for example <code>objectId: 757533…</code></li>
      </ul>
      ...
      <KV>
        ...
        <KVRow label="Cancel">Ask Maestro to cancel a running instance.</KVRow>
        ...
        <KVRow label="Open in Docusign">Deep-link to the instance in Docusign Maestro.</KVRow>
      </KV>

      <h3>The four instance states</h3>
      <p>
        After Baton triggers a workflow, Maestro runs it independently. Baton periodically polls Maestro for
        instance status so you can monitor from inside Baton without opening Docusign. Every instance is in one
        of four states:
      </p>
      ...
      <Callout type="note" title="Status can be briefly stale">
        For long-running instances, Maestro can take a few minutes to update. The panel may show slightly stale
        data while Baton polls for the latest status — it catches up on its own.
      </Callout>
      ...
      <ul>
        <li><strong>Verification fails</strong> (the HMAC or Basic Auth signature didn't match): the Action is marked failed, nothing fires to Maestro, and it's logged.</li>
        ...
        <li><strong>Routing fails</strong> (the Maestro API returned an error): Baton retries automatically up to <strong>6 times</strong> with exponential backoff. If all retries are exhausted, the Action is marked <Badge color="red">upstream_error</Badge> and an <strong>Automation Failed</strong> notification fires.</li>
      </ul>
      ...
      <Callout type="note" title="Retries are not gated by plan">
        The same retry schedule applies on every tier. Automatic routing retries are part of Baton's core
        behavior, not a paid add-on.
      </Callout>
