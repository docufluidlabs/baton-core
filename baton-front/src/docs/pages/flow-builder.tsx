  ...
  return (
    <>
      ...
      <p>
        ...
        through the automation, into the Docusign workflow.
      </p>
      ...
      <p>
        The right card is the target Docusign workflow. It shows the workflow icon and name, live counts
        (completed / total), and a <strong>View Instances</strong> button. View Instances lists every workflow
      ...
      <Steps>
        ...
        <Step title="Target Docusign workflow">
          {' '}Choose the workflow this automation triggers. The dropdown is populated from your synced Docusign
          workflows (sync them on the <DocLink to="workflows">Workflow Checker</DocLink> page).
        </Step>
        <Step title="Review the API parameters">
          {' '}The <strong>Workflow Builder API Parameters</strong> section lists the parameters the target
          workflow expects, so you know which fields the source payload needs to carry - and lets you map each
          one to a payload path, a static value, or a template.
        </Step>
