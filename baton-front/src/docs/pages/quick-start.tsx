...
export default function QuickStart() {
  return (
    <>
      <h1>Quick start</h1>
      <Lead>Connect Docusign, sync your Maestro workflows, add a source platform, and wire your first live automation — in the order an admin actually does it.</Lead>

      <h2>Before you begin</h2>
      <p>This walkthrough assumes you have everything Baton needs to reach both ends of an automation.</p>

      <Callout type="note" title="What you need">A Docusign account with at least one Maestro workflow, and admin access to your source platform (such as HubSpot) so you can paste a webhook URL into its settings.</Callout>

      <h2>Step 1 — Sign in and create your organization</h2>
      <p>Signing in creates or joins an organization. Everyone who signs in is an admin of their org, so there are no extra permissions to set up.</p>
      ...
      <h2>Step 3 — Sync your Maestro workflows</h2>
      <p>After Docusign is connected, Baton syncs your Maestro workflows from Docusign.</p>

      <Steps>
        <Step title="Open Workflow Checker"> Go to the <DocLink to="workflows">Workflow Checker</DocLink> (its on-screen header reads "Maestro Workflows").</Step>
      ...
      <Steps>
        ...
        <Step title="Launch"> Click <strong>Launch</strong>. A successful manual run confirms the workflow is ready before you connect a live source.</Step>
      </Steps>
      ...
      <p>An automation pairs the platform's webhook URL with a target Maestro workflow.</p>
      ...
      <Steps>
        ...
        <Step title="Choose the Target Maestro Workflow"> Select which workflow this automation should launch.</Step>
      ...
      <FlowStrip>
        ...
        <FlowNode k="In Maestro" t="Instance launches" d="A matching Maestro instance starts running." />
        <FlowNode k="In Flow Builder" t="Live counts update" d="The automation card updates; use View Instances to watch the run." />
      </FlowStrip>

      <Callout type="tip" title="Watch the run from the card">On the <DocLink to="flow-builder">Flow Builder</DocLink> card, live counts update as the run progresses. Click <strong>View Instances</strong> to open the specific Maestro instance and follow its status.</Callout>

      <h2>Where to go next</h2>
      <Cards>
        <Card to="how-it-works" title="How Baton works">Understand the verify-then-route model and the boundary between Baton and Maestro.</Card>
