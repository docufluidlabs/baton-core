...
export default function CustomWebhook() {
  return (
    <>
      ...
      <Lead>A custom POST endpoint lets any source system that can send JSON — an internal app, a vendor portal, an IoT gateway — launch a Maestro workflow through Baton, even if it is not in the catalog.</Lead>
      ...
      <FlowStrip>
        <FlowNode k="You" t="Create the endpoint" d="Give it a name, a target Maestro workflow, and a payload field path." />
      ...
      <Steps>
        ...
        <Step title="Choose the target workflow"> The Maestro workflow this endpoint should launch.</Step>
      ...
      <TableWrap>
        <table>
          ...
          <tbody>
            ...
            <tr><td><strong>target workflow</strong></td><td>The Maestro workflow to launch on each accepted event.</td></tr>
      ...
      <p>The payload field path tells Baton where to find the record ID inside the JSON your system posts. It is written in dot notation, and the leading <code>{'$.'}</code> is optional. The extracted value is then passed to Maestro under the parameter name the target workflow expects.</p>
