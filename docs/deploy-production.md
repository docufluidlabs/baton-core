...
1. **Docusign app:** in-app **Connections → Docusign** has the field-by-field guide (integration key, secret key, and the redirect URI - which must be your public `API_URL`, not localhost; the UI warns if it isn't). Ignore Docusign's RSA Keypairs section: those belong to the JWT grant, which Baton does not use. Set the Connect HMAC key (`DOCUSIGN_CONNECT_HMAC_KEY`) so workflow status callbacks are signature-verified.
